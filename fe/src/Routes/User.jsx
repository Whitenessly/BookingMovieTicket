...
const User = () => {
    ...
    React.useEffect(() => {
        fetch(`http://localhost:8080/user/${sessionKey}`)
            ...
            {(user) ?
                <div className='p-7 pb-23 h-full flex flex-col justify-between'>
                    ...
                    <div className='flex flex-col gap-4'>
                        {(user.role === 'staff') ?
                            <Link to='/staff/home'><div className='w-full flex flex-row justify-center items-center gap-3 text-2xl border-b-2 border-r-2 border-gray-200 bg-pink-500 py-3 rounded-lg'>Staff Mode <FaArrowRight /></div></Link>
                            : null
                        }
